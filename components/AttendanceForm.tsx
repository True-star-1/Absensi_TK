...
import { Calendar, Users, Save, School, AlertCircle } from 'lucide-react';
...
const AttendanceForm: React.FC<AttendanceFormProps> = ({ classes, students, attendance, setAttendance }) => {
  ...
  // Helper: Status yang WAJIB ada keterangan (Sakit dan Izin)
  ...
  useEffect(() => {
    if (selectedClassId && selectedDate) {
      ...
      filteredStudents.forEach(s => {
        ...
        if (record) {
          // Normalisasi status saat loading (Contoh: 'sakit' -> 'Sakit')
  ...
  const updateStatus = (studentId: string, status: AttendanceStatus) => {
    setCurrentSession(prev => ({
      ...
      [studentId]: { 
        ...prev[studentId], 
        status, 
        // Pertahankan note jika sudah ada agar tidak hilang saat ganti status antar Sakit/Izin
  ...
  const handleSave = async () => {
    ...
    // 2. Validasi Keterangan (Sakit & Izin)
      ...
      {selectedClassId ? (
        <div className="space-y-4">
          {filteredStudents.map((student) => {
            ...
            return (
              <div key={student.id} className={`bg-white p-6 rounded-[32px] border transition-all duration-300 ${isError ? 'border-red-300 bg-red-50/20 shadow-lg shadow-red-50' : 'border-slate-100 shadow-sm'}`}>
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="flex items-center gap-4">
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-xl border ${isError ? 'bg-red-100 text-red-600 border-red-200' : 'bg-slate-50 text-indigo-600 border-slate-100'}`}>
                      {student.name.charAt(0)}
                    </div>
                    <div>
                      <h3 className={`font-black text-lg ${isError ? 'text-red-700' : 'text-slate-800'}`}>{student.name}</h3>
                      <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">NIS: {student.nis}</p>
                    </div>
                  </div>
                ...
                {needsNote && (
                  <div className="mt-5 animate-in slide-in-from-top-2">
                    <label className={`text-[10px] font-black uppercase tracking-widest mb-1.5 block ${isError ? 'text-red-600' : 'text-slate-400'}`}>
                      Keterangan Alasan {sess.status} <span className="text-red-500">*</span>
                    </label>
                    <input 
                      placeholder={`Misal: Sakit demam tinggi / Ada acara pernikahan keluarga...`}
                      className={`w-full p-4 rounded-2xl border outline-none font-medium text-sm italic transition-all ${isError ? 'border-red-300 bg-red-50 focus:ring-2 focus:ring-red-100 placeholder-red-300' : 'bg-slate-50 border-slate-100 focus:ring-2 focus:ring-indigo-100'}`}
                      value={sess?.note || ''}
                      onChange={(e) => updateNote(student.id, e.target.value)}
                    />
                    {isError && <p className="text-[10px] text-red-500 font-black mt-2 animate-pulse uppercase">Wajib diisi agar data tersimpan!</p>}
                  </div>
                )}
              </div>
            );
          ...
          <div className="fixed bottom-24 left-0 right-0 px-4 md:static md:px-0 md:pt-10 flex justify-center z-40">
            <button 
              onClick={handleSave}
              disabled={isProcessing}
              className="bg-indigo-600 text-white px-16 py-5 rounded-full font-black text-lg shadow-2xl shadow-indigo-200 hover:bg-indigo-700 transition-all flex items-center gap-3 active:scale-95 disabled:opacity-50"
            >
              <Save size={24} />
        ...
        <div className="py-24 text-center bg-white rounded-[48px] border-2 border-slate-100 border-dashed">
          ...
          <p className="text-slate-400 font-bold max-w-xs mx-auto">Pilih kelas terlebih dahulu untuk memuat daftar anak didik.</p>
        </div>
      )}
    </div>
  );
...
const StatusBtn: React.FC<{ label: string, active: boolean, onClick: () => void, color: string }> = ({ label, active, onClick, color }) => {
  const styles: Record<string, string> = {
    emerald: active ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-100' : 'bg-slate-50 text-slate-400 hover:bg-emerald-50 hover:text-emerald-600',
    blue: active ? 'bg-blue-500 text-white shadow-lg shadow-blue-100' : 'bg-slate-50 text-slate-400 hover:bg-blue-50 hover:text-blue-600',
    amber: active ? 'bg-amber-500 text-white shadow-lg shadow-amber-100' : 'bg-slate-50 text-slate-400 hover:bg-amber-50 hover:text-amber-600',
    red: active ? 'bg-red-500 text-white shadow-lg shadow-red-100' : 'bg-slate-50 text-slate-400 hover:bg-red-50 hover:text-red-600',
  };
  return (
    <button onClick={onClick} className={`px-5 py-2.5 rounded-2xl font-black text-xs transition-all transform active:scale-90 ${styles[color]}`}>
      {label}
    </button>
  );
};
